...
class PreActBlock_NF(nn.Module):
    '''Pre-activation version of the BasicBlock.'''
    expansion = 1

    def __init__(self, in_channels, out_channels, stride, dropout, neuron: callable = None, **kwargs):
        super(PreActBlock_NF, self).__init__()
        whether_bias = True
        #self.bn1 = nn.BatchNorm2d(in_channels)
    ...
    def forward(self, x):
        x = self.relu1(x)
        out = self.conv1(x)
        out = self.conv2(self.dropout(self.relu2(out)))
        out = out + self.shortcut(x)
        return out
...
class PreActResNet_NF(nn.Module):

    def __init__(self, block, num_blocks, num_classes, dropout, neuron: callable = None, **kwargs):
        super(PreActResNet_NF, self).__init__()
        self.num_blocks = num_blocks
...
def spiking_resnet18_NF(neuron: callable = None, num_classes=10,  neuron_dropout=0, **kwargs):
    return PreActResNet_NF(PreActBlock_NF, [2, 2, 2, 2], num_classes, neuron_dropout, neuron=neuron, **kwargs)
